...
class ParquetTransformer:
    ...
    def validate_and_transform(self, ticker: str, df: pd.DataFrame) -> list:
        """
        Memvalidasi data, lalu menyimpannya sebagai file Parquet terpartisi per bulan.
        ...
        # 1. Pengecekan Kualitas Data (Data Quality Checks)
        # Hapus baris di mana volume 0 (mungkin suspend)
        initial_len = len(df)
        df = df[df['volume'] > 0]

        # Hapus baris dengan NaN di adj_close
        df = df.dropna(subset=['adj_close'])

        # Data Quality Check: Pastikan harga masuk akal (mencegah halusinasi API)
        ...
        # 2. Tambahkan kolom tanggal untuk partisi
        df = df.copy()
        ...
        # 3. Simpan ke Parquet terpartisi per BULAN
